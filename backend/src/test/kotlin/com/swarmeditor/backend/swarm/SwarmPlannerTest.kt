...
class SwarmPlannerTest {
    ...
    fun `pi planner repairs invalid output and closes its planning session`() = runTest {
        ...
        assertContains(prompts.first(), "Dependency-cluster evidence describes coupled repository files")
        assertContains(prompts.first(), "Retire the highest-impact uncertainty first")
        assertContains(prompts.first(), "Do not create one task per file")
        assertContains(prompts.first(), "end-to-end data path explicit")
        assertContains(prompts.first(), "downstream handoff")
