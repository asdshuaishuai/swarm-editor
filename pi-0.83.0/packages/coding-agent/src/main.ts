...
import { createStdioToolBrokerTools, StdioToolBrokerClient } from "./core/tools/stdio-tool-broker.ts";
...
export async function main(args: string[], options?: MainOptions) {
	...
	if (toolBrokerMode) {
		if (toolBrokerMode !== "stdio-v1") {
			console.error(chalk.red(`Error: Unsupported SWARM_PI_TOOL_BROKER mode: ${toolBrokerMode}`));
			process.exit(1);
		}
		if (appMode !== "rpc") {
			console.error(chalk.red("Error: SWARM_PI_TOOL_BROKER=stdio-v1 requires RPC mode"));
			process.exit(1);
		}
		try {
			toolBrokerClient = new StdioToolBrokerClient({
				sessionNonce: process.env.SWARM_PI_TOOL_BROKER_NONCE?.trim() ?? "",
			});
		} catch (brokerError) {
			console.error(chalk.red(`Error: ${brokerError instanceof Error ? brokerError.message : String(brokerError)}`));
			process.exit(1);
		}
	}
	...
	if (appMode === "rpc") {
		printTimings();
		await runRpcMode(runtime, { toolBrokerClient });
